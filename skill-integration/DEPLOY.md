...
# 备份将要修改的现有文件
BAK="/tmp/r-template-index.bak.$(date +%Y%m%d_%H%M%S).md"
cp ~/.claude/skills/nature-figure/references/r-template-index.md "$BAK"
echo "备份完成：$BAK"
```
...
```bash
# 与备份对比，确认只有末尾追加
diff "$BAK" ~/.claude/skills/nature-figure/references/r-template-index.md   # 确认仅末尾新增
```
...
# rollback if needed: cp "$BAK" ~/.claude/skills/nature-figure/references/r-template-index.md
cp "$BAK" ~/.claude/skills/nature-figure/references/r-template-index.md

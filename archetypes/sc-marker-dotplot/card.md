...
- **点颜色** = 该基因在各细胞类型的平均表达 z-score（跨细胞类型归一化，蓝→高，红→低，配色来自 `nature_div`）
...
7. **z-score 反映"相对特异"而非"绝对高表达"**：一个基因在所有 celltype 均低表达时，z-score 仍会有蓝色峰值（相对最高那列），但绝对均值可能接近于 0（log1p 接近 0.4 本底）。报告时务必同时注明绝对均值范围，避免将背景噪音误读为特异高表达。
